...
import ProjectNaming from '../utils/projectNaming'
...
export default function SimulatorTab({ projects, onAdd, onDelete, onUpdate, customFunctions, onCustomFunctionsChange }) {
  ...
  const loadAvailableEnvironments = async () => {
    try {
      ...
      // Set current environment as default selection
      ...
      <div>
        ...
        <div style={{ background: '#FFF3E0', padding: '12px', borderRadius: '6px', marginBottom: '12px', border: '1px solid #FFB74D' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', marginBottom: '8px', flexWrap: 'wrap' }}>
            ...
            <button
              onClick={playLoop}
              disabled={recordingNotes.length === 0 || !simulatorState.isRunning || isRecording || isPlayingLoop}
              style={{
                padding: '6px 10px',
                background: '#4CAF50',
                color: '#fff',
                ...
                fontSize: '10px',
                fontWeight: '600',
                cursor: (recordingNotes.length > 0 && simulatorState.isRunning && !isRecording && !isPlayingLoop) ? 'pointer' : 'not-allowed',
                opacity: (recordingNotes.length > 0 && simulatorState.isRunning && !isRecording) ? 1 : 0.5,
              }}
            >
            ...
            <button
              onClick={clearLoop}
              disabled={recordingNotes.length === 0 || !simulatorState.isRunning}
              style={{
                padding: '6px 10px',
                background: '#F44336',
                color: '#fff',
                ...
                fontSize: '10px',
                fontWeight: '600',
                cursor: (recordingNotes.length > 0 && simulatorState.isRunning) ? 'pointer' : 'not-allowed',
                opacity: (recordingNotes.length > 0 && simulatorState.isRunning) ? 1 : 0.5,
              }}
            >
              🗑 Clear
            </button>
